...
async function enhanceProperties() {
  ...
  console.log("\n🎉 Property enhancement complete!");
}

async function createDynamicPricing() {
  ...
  for (const property of properties) {
    ...
    console.log(`✅ Created pricing for property ${property.id}: ${property.name}`);
  }

  console.log("\n🎉 Dynamic pricing creation complete!");
}

async function main() {
  try {
    await enhanceProperties();
